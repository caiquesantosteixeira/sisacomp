...
@IonicPage()
@Component({
  selector: 'page-reclamacoes',
  templateUrl: 'reclamacoes.html',
})
export class ReclamacoesPage {
  ...
  delete(reclamacao:reclamacao){

    this.Abrecarregamento("carreganddo");
  
      this.reclamacaoProvider.DeleteReclamacao(reclamacao).subscribe(
        data => {
          this.FechaCarregamento();
          const response = (data as any);
          const objeto_retorno = JSON.parse(response._body);
          ...
          if(response.status == 200){
            ...
            this.reclamacoes = this.reclamacoes.filter(a => a.idReclamacao != reclamacao.idReclamacao);
          }else{
            if(response.status == 500){
              this.exibirMensagem('Erro interno no servidor');
            }else{
